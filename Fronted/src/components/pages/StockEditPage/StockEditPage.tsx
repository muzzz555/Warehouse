  ...
  return (
    <Box>
      <Formik
        ...
        enableReinitialize   // Track value when change in Formik
        initialValues={stockEditReducer.result ? stockEditReducer.result : initialValues}
        onSubmit={(values, { setSubmitting }) => {
          // alert(JSON.stringify(values));
          let formData = new FormData();
          formData.append("id", String(values.id)); // where id
          formData.append("name", values.name);
          formData.append("price", String(values.price));
          formData.append("stock", String(values.stock));

          if (values.file) {
            formData.append("image", values.file); // check when not update image give old image
          }
